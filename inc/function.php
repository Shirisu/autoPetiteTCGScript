...
function member_level_up($member_id)
{
    ...
    if (($row_member['member_level'] != $row_level['member_level_id']) && ($row_member['member_level'] < $row_level['member_level_id'])) {
        ...
        $inserted_cards_text = TRANSLATIONS[$GLOBALS['language']]['general']['text_level_up_reward'] . ': ' . implode(', ',
                $_SESSION['insert_cards']);
        insert_log(TRANSLATIONS[$GLOBALS['language']]['general']['text_level_up'] . ' - ' . sprintf('%02d',
                $row_level['member_level_id']), $inserted_cards_text, $member_id);

        mysqli_query($link,
            "UPDATE member SET member_level = '" . $row_level['member_level_id'] . "' WHERE member_id = '" . $member_id . "' LIMIT 1");

        alert_box(
            TRANSLATIONS[$GLOBALS['language']]['general']['text_level_up'] . ': ' . TRANSLATIONS[$GLOBALS['language']]['general']['text_level_up_reward'] . ': ' . implode(', ',
                $_SESSION['insert_cards'])
            , 'success');
    }
}
...
function insert_cards($member_id, $quantity)
{
    ...
    if (mysqli_num_rows($result)) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            mysqli_query($link,
                "INSERT INTO member_cards (member_cards_carddeck_id, member_cards_number, member_cards_member_id) VALUES ('" . $row['carddeck_id'] . "','" . $cardnumber . "','" . $member_id . "')") or die(mysqli_error($link));
            array_push($cardarray, $row['carddeck_name'] . sprintf("%02d", $cardnumber));
            $_SESSION['insert_cards'] = $cardarray;
        }
        mysqli_query($link,
            "UPDATE member SET member_cards = member_cards + '" . $quantity . "' WHERE member_id = '" . $member_id . "' LIMIT 1") or die(mysqli_error($link));
    }
}
...
function insert_specific_cards($member_id, $carddeck_id, $card_number)
{
    ...
    if (mysqli_num_rows($result)) {
        mysqli_query($link,
            "INSERT INTO member_cards (member_cards_carddeck_id, member_cards_number, member_cards_member_id) VALUES ('" . $carddeck_id . "','" . $card_number . "','" . $member_id . "')") or die(mysqli_error($link));
        mysqli_query($link,
            "UPDATE member SET member_cards = member_cards + 1 WHERE member_id = '" . $member_id . "' LIMIT 1") or die(mysqli_error($link));
    }
}
...
function insert_wish($member_id, $quantity)
{
    ...
    mysqli_query($link,
        "UPDATE member SET member_wish = member_wish + '" . $quantity . "' WHERE member_id = '" . $member_id . "' LIMIT 1") or die(mysqli_error($link));
}
...
function insert_currency($member_id, $quantity)
{
    ...
    mysqli_query($link,
        "UPDATE member SET member_currency = member_currency + '" . $quantity . "' WHERE member_id = '" . $member_id . "' LIMIT 1") or die(mysqli_error($link));
}
...
function insert_shop_random($member_id, $quantity)
{
    ...
    if (get_member_currency($_SESSION['member_id']) / TCG_SHOP_CURRENCY_FOR_RANDOM >= $quantity) {
        ...
        mysqli_query($link,
            "UPDATE member SET member_currency = member_currency - '" . $currency_spent . "' WHERE member_id = '" . $member_id . "' LIMIT 1") or die(mysqli_error($link));
        insert_cards($member_id, $quantity);
        $inserted_cards_text = TRANSLATIONS[$GLOBALS['language']]['shop']['text_you_spent'] . ' ' . $currency_spent . ' ' . TCG_CURRENCY . ' ' . TRANSLATIONS[$GLOBALS['language']]['shop']['text_and_got_following'] . ': ' . implode(', ',
                $_SESSION['insert_cards']);
...
function insert_shop_card($member_id, $carddeck_id, $card_number)
{
    ...
    if (get_member_wish($_SESSION['member_id']) > 0) {
        ...
        if (mysqli_num_rows($result_carddeck)) {
            ...
            $carddeck_name = $row_carddeck['carddeck_name'];
            mysqli_query($link,
                "UPDATE member SET member_wish = member_wish - 1 WHERE member_id = '" . $member_id . "' LIMIT 1") or die(mysqli_error($link));
            insert_specific_cards($member_id, $carddeck_id, $card_number);
            $inserted_card_text = TRANSLATIONS[$GLOBALS['language']]['shop']['text_you_spent'] . ' 1 ' . TCG_WISH . ' ' . TRANSLATIONS[$GLOBALS['language']]['shop']['text_and_got_following'] . ': ' . $carddeck_name . sprintf('%02d',
                    $card_number);
