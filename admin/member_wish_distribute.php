...
if (isset($_SESSION['member_rank']) && ($_SESSION['member_rank'] == 1 || $_SESSION['member_rank'] == 2 || $_SESSION['member_rank'] == 4)) {
    ...
    if (isset($_POST['member_id']) && isset($_POST['quantity']) && isset($_POST['topic'])) {
        ...
        $inserted_wish_text = TRANSLATIONS[$GLOBALS['language']]['admin']['text_log_distribute_wish'].': '.$quantity;

        insert_log(TRANSLATIONS[$GLOBALS['language']]['admin']['text_log_distribute_wish_topic'], $inserted_wish_text, $member_id);
        $text = $inserted_wish_text.' - '.TRANSLATIONS[$GLOBALS['language']]['admin']['text_log_distribute_wish_topic'].': '.$topic;
    ...
    ?>
    <form action="/administration/distributewish" method="post">
