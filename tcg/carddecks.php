...
if (isset($_SESSION['member_rank'])) {
    ...
    if (isset($category_id)) {
        $sql_carddeck_cat_name = "SELECT carddeck_cat_name
                                  ...
                                  LIMIT 1";
        $result_carddeck_cat_name = mysqli_query($link, $sql_carddeck_cat_name);
            ...
            if ($count_carddeck) {
                ...
                <table id="admin_member_edit_table" data-mobile-responsive="true">
                    ...
                    while ($row_carddeck = mysqli_fetch_assoc($result_carddeck)) {
                        $sql_on_wishlist = "SELECT member_wishlist_id
                                            ...
                                            LIMIT 1";
                        $result_on_wishlist = mysqli_query($link, $sql_on_wishlist);
                        $is_on_wishlist = mysqli_num_rows($result_on_wishlist) > 0;
                        ?>
                        <tr>
                            <td><img src="<?php echo TCG_CARDS_FOLDER.'/'.$row_carddeck['carddeck_name'].'/'.$row_carddeck['carddeck_name'].'master.'.TCG_CARDS_FILE_TYPE; ?>"/></td>
                            <td><a href="/carddecks/<?php echo $row_carddeck['carddeck_name']; ?>"><?php echo $row_carddeck['carddeck_name']; ?></a></td>
                            <td><?php echo $row_carddeck['carddeck_series']; ?></td>
                            <td><?php echo $row_carddeck['carddeck_cat_name']; ?> <i class="fas fa-chevron-right"></i> <?php echo $row_carddeck['carddeck_sub_cat_name']; ?></td>
                            ...
                            <td>
                                <span title="<?php echo TRANSLATIONS[$GLOBALS['language']]['general']['text_add_to_wishlist']; ?>">
